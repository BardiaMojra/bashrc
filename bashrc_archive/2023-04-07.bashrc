...
# ----------------------------------------------------------------->> log & load
alias loadallcfgs='loadbash &&\
                   loadtmux_conf &&\
                   loadgitconfig'
alias logallcfgs='logbash &&\
                  logtmux_conf &&\
                  loghistory &&\
                  loggitconfig'
...
alias loadtmux_conf='cp ~/git/bashrc/.tmux.conf ~/.tmux.conf'
alias logtmux_conf='cp ~/.tmux.conf ~/git/bashrc/.tmux.conf &&\
cp  ~/git/bashrc/.tmux.conf  ~/git/bashrc/tmux_conf_archive/$(date +%F).tmux.conf &&\
cd ~/git/bashrc/ && git status'
...
alias loadgitconfig='cp ~/git/bashrc/.gitconfig ~/.gitconfig &&\
cp ~/git/bashrc/.git-credentials ~/.git-credentials'
...
alias gitresconflict="git reset --mixed HEAD~1 && git add . && \
  git pull && git restore --staged . && git add -A && \
  git commit -m 'Resolved branch heads conflict with gitresconflict on $(date +%F).' &&\
  git push"
...
alias gigcondaf='cp ~/git/cheatsheets/conda_env_temp.yml .'
#alias gigvenv
#alias gig

# -------------------------------------------------------------->> get proj temp
# /git/cheatsheets/***_proj/
alias gigpy_proj='cp -r ~/git/cheatsheets/py_proj/* .'
alias gigmatlab_proj='cp -r ~/git/cheatsheets/matlab_proj/* .'
alias gigtex_proj='cp -r ~/git/cheatsheets/tex_proj/* .'
alias gigrosd_proj='cp -r ~/git/cheatsheets/rosd_proj/* .'
...
# ---------------------------------------------------------------->> permissions
# Basic syntax:
# for ROS ws use chMOD not chown
# sudo chmod -R 777 dlo_ws/ 
# sudo chown -R smerx:smerx dlo_ws/
